...
def get_loans():
    ...
    q = "SELECT * FROM loans WHERE user_id=%s"
    p = [uid()]
...
def add_loan():
    ...
    execute_query("""
        INSERT INTO loans (user_id, borrower_name, principal, interest_rate, loan_date_bs,
        loan_date_ad, duration_months, interest_amount, total_payable, status, remarks)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
    """, (uid(), d['borrower_name'], principal, rate, d.get('loan_date_bs',''),
...
def update_loan(id):
    ...
    execute_query("""
        ...
        status=%s, remarks=%s WHERE id=%s AND user_id=%s
    """, (d['borrower_name'], principal, rate, d.get('loan_date_bs',''),
          d.get('loan_date_ad', None), months, interest, total,
          d.get('status','active'), d.get('remarks',''), id, uid()), fetch=False)
    return jsonify({'success': True, 'message': 'Loan updated'})

@loan_bp.route('/api/loan/<int:id>', methods=['DELETE'])
def delete_loan(id):
    execute_query("DELETE FROM loans WHERE id=%s AND user_id=%s", (id, uid()), fetch=False)
