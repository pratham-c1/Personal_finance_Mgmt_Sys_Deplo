...
def login():
    ...
    user = execute_one("SELECT * FROM settings WHERE password_hash = %s LIMIT 1", (hashed,))
    if user:
...
def change_password():
    ...
    user = execute_one("SELECT * FROM settings WHERE password_hash = %s", (old_hashed,))
    if not user:
        return jsonify({'success': False, 'message': 'Old password incorrect'}), 400
    new_hashed = hash_password(new_pw)
    execute_query("UPDATE settings SET password_hash = %s", (new_hashed,), fetch=False)
