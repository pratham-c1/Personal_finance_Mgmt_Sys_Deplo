...
def get_baby_expense():
    data = execute_query("SELECT * FROM baby_expenditure WHERE user_id=%s ORDER BY date_ad DESC", (uid(),))
    result = []
    for r in data:
        d = dict(r)
...
def add_baby_expense():
    d = request.get_json()
    execute_query("""
        INSERT INTO baby_expenditure (user_id, date_bs, date_ad, year_bs, month_bs, month_num, category, particular, amount, remarks)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
    """, (uid(), d['date_bs'], d['date_ad'], d['year_bs'], d['month_bs'], d['month_num'],
...
def update_baby_expense(id):
    ...
    execute_query("""
        ...
        category=%s, particular=%s, amount=%s, remarks=%s WHERE id=%s AND user_id=%s
    """, (d['date_bs'], d['date_ad'], d['year_bs'], d['month_bs'], d['month_num'],
          d['category'], d.get('particular',''), float(d['amount']), d.get('remarks',''), id, uid()), fetch=False)
...
def delete_baby_expense(id):
    execute_query("DELETE FROM baby_expenditure WHERE id=%s AND user_id=%s", (id, uid()), fetch=False)
    return jsonify({'success': True, 'message': 'Baby expense deleted'})

@baby_bp.route('/api/baby/gift', methods=['GET'])
def get_baby_gifts():
    data = execute_query("SELECT * FROM baby_gifts WHERE user_id=%s ORDER BY date_ad DESC", (uid(),))
    result = []
    for r in data:
        d = dict(r)
        if d.get('date_ad'): d['date_ad'] = str(d['date_ad'])
...
def add_baby_gift():
    d = request.get_json()
    execute_query("""
        INSERT INTO baby_gifts (user_id, person_name, amount, date_bs, date_ad, remarks)
        VALUES (%s,%s,%s,%s,%s,%s)
    """, (uid(), d['person_name'], float(d['amount']), d.get('date_bs',''), d.get('date_ad', None), d.get('remarks','')), fetch=False)
...
def update_baby_gift(id):
    d = request.get_json()
    execute_query(
        "UPDATE baby_gifts SET person_name=%s, amount=%s, date_bs=%s, date_ad=%s, remarks=%s WHERE id=%s AND user_id=%s",
        (d['person_name'], float(d['amount']), d.get('date_bs',''), d.get('date_ad', None), d.get('remarks',''), id, uid()), fetch=False)
    return jsonify({'success': True, 'message': 'Gift updated'})

@baby_bp.route('/api/baby/gift/<int:id>', methods=['DELETE'])
def delete_baby_gift(id):
    execute_query("DELETE FROM baby_gifts WHERE id=%s AND user_id=%s", (id, uid()), fetch=False)
